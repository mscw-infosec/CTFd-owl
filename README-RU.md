...
3. Адаптировано для обоих режимов CTFd: индивидуального и командного. В командном режиме запускается один инстанс на команду или один на каждого пользователя.
...
-   `owl.label.ssh_username=ctf` - используется только когда `owl.label.conntype=ssh` (показывается как `ssh ctf@ip -p port` в карточке задания)

Отображение данных для подключения изменены для `nс`, `ssh`.
...
-   Пример обычной задачи приведён в `CTFd/plugins/ctfd-owl/source/sanity-task`. Вы можете создать свою задачу на её основе.
-   Пример задачи с динамическим флагом приведён в `CTFd/plugins/ctfd-owl/source/dynamic-task`. Вы можете создать свою задачу на её основе.
