...
4. Поддерживаются статические (plaintext или regex), динамические и полу-динамические (шаблонные) флаги.
5. Переменная окружения `FLAG` всегда прокидывается в контейнеры при старте: в статическом режиме это заданный флаг, в динамическом/полу-динамическом — уникальный флаг для инстанса.
6. Всё, что связано с контейнером (включая FRP), настраивается декларативно через лейблы Docker Compose.
7. Поддерживаются разные типы сообщений (toasts, modals) о статусе контейнеров, а также совместимость со старыми core-based темами и новыми.
...
- Пример задачи с динамическим флагом приведён в `CTFd/plugins/ctfd-owl/source/tY0u_F1nd_M3asks/dynamic-task`.
...
- `ctf{Y0u_F1nd_M3_$[!gen!]}` — `$[!gen!]` будет заменён на случайную строку длины **Semi-Dynamic Flag Length**.
- `ctf{Y0u_F1nd_M3_$[!gen:06!]}` — сгенерирует 6 символов (перекрывает настройку длины для конкретного плейсхолдера).
